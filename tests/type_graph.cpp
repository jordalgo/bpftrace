       ...
       ExpectedAST{ Program().WithProbe(
           Probe(_,
                 { AssignVarStatement(Variable("$a").WithType(
                                          SizedType(Type::string).WithSize(4)),
                                      _) })) });
       ...
       ExpectedAST{ Program().WithProbe(
           Probe(_,
                 { VarDeclStatement(IntVar("$y", 1)),
                   AssignVarStatement(IntVar("$z", 1), _),
       ...
       ExpectedAST{ Program().WithProbe(
           Probe(_,
                 { VarDeclStatement(IntVar("$h", 1)),
                   ...
                   AssignVarStatement(IntVar("$i", 1), _),
       ...
       ExpectedAST{ Program().WithProbe(Probe(
           ...
           { ExprStatement(If(
               ...
               Block({ AssignVarStatement(
                   Variable("$x").WithType(SizedType(Type::string).WithSize(4)),
                   _) }))) })) });
  test(
          ...
          Probe(_,
                { VarDeclStatement(IntVar("$y", 1, true)),
                  ExprStatement(If(
                      ...
                      Block({ AssignVarStatement(
                          IntVar("$x", 2, true),
                          Binop(Operator::PLUS, IntVar("$y", 1, true), _)) }))),
                  AssignVarStatement(IntVar("$y", 1, true), _) })) });

  // Invalid binop
  test(R"(begin { $a = "str" + true; })",
       ExpectedAST{ Program().WithProbe(
           Probe(_,
                 { AssignVarStatement(
                     Variable("$a").WithType(SizedType(Type::none)), _) })) });
}

TEST_F(TypeGraphTest, variable_typeof)
{
...
{
  test(R"(begin { $a = 1; $b = typeinfo($a); })",
       ExpectedAST{ Program().WithProbe(Probe(
           _,
           { AssignVarStatement(IntVar("$a", 1), _),
             AssignVarStatement(
                 Variable("$b").WithType(SizedType(Type::tuple)),
                 Tuple({ Integer(0), String("int"), String("uint8") })) })) });
  ...
  test(R"(begin { $a = 1; $b = typeinfo({ $z = (uint64)2; $z }); })",
       ExpectedAST{ Program().WithProbe(Probe(
           ...
           { AssignVarStatement(IntVar("$a", 1), _),
             AssignVarStatement(
                 Variable("$b").WithType(SizedType(Type::tuple)),
                 Tuple({ Integer(0), String("int"), String("uint64") })) })) });
       ...
       ExpectedAST{ Program().WithProbe(
           Probe(_,
                 { VarDeclStatement(IntVar("$a", 8, true)),
                   AssignVarStatement(
                       Variable("$b").WithType(SizedType(Type::tuple)),
                       Tuple({ Integer(0), String("int"), String("int64") })),
                   _,
                   _ })) });
...
{
  test(
      R"(begin { let $c; $a = 1; if comptime (typeinfo($a).2 == "uint64") { $c = (int64)2; } $a = (uint64)2; })",
      ExpectedAST{ Program().WithProbe(
          Probe(_,
                { VarDeclStatement(IntVar("$c", 8, true)),
                  AssignVarStatement(IntVar("$a", 8), _),
  ...
  // The type of $c does not change and the assignment becomes an error in a
  // later pass
  test(
      R"(begin { let $c; if comptime (typeinfo($c).2 == "uint32") { $c = (uint64)2; } $c = (uint32)1; })",
      ExpectedAST{ Program().WithProbe(Probe(
          _,
          { VarDeclStatement(IntVar("$c", 4)),
            ExprStatement(Block({ AssignVarStatement(IntVar("$c", 4), _) })),
            AssignVarStatement(IntVar("$c", 4), _) })) });

  test(
      R"(begin { let $c; $a = 1; if comptime (typeinfo($a).2 == "uint64") { let $d; if comptime (typeinfo($d).2 == "int64") { $c = (int16)3; } $d = (int64)2; } $a = (uint64)2; })",
      ExpectedAST{ Program().WithProbe(
          Probe(_, { VarDeclStatement(IntVar("$c", 2, true)), _, _, _ })) });

  test(
      R"(begin { let $c; let $e; let $d; $a = 1; if comptime (typeinfo($a).2 == "uint64") { if comptime (typeinfo($d).2 == "int64") { $c = (int16)3; } $e = (int32)1; } $a = (uint64)2; if comptime (typeinfo($e).2 == "int32") { $d = (int64)3; } })",
      ExpectedAST{ Program().WithProbe(Probe(
          _, { VarDeclStatement(IntVar("$c", 2, true)), _, _, _, _, _, _ })) });

  test(R"(begin { let $c; if comptime (typeinfo($c).1 == "int") { 1 } })",
       Error{ R"(
stdin:1:20-54: ERROR: Unable to resolve comptime expression
begin { let $c; if comptime (typeinfo($c).1 == "int") { 1 } }
                   ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
)" });
...
TEST_F(TypeGraphTest, map_value_basic)
{
  ...
  test(R"(begin { @a = "str"; })",
       ExpectedAST{ Program().WithProbe(
           Probe(_,
                 { AssignMapStatement(Map("@a").WithType(
                                          SizedType(Type::string).WithSize(4)),
                                      _,
                                      _) })) });
  ...
  test(
      R"(begin { @a = "hi"; @a = "hello world"; })",
      ExpectedAST{ Program().WithProbe(Probe(
          _,
          { AssignMapStatement(
                Map("@a").WithType(SizedType(Type::string).WithSize(12)), _, _),
            AssignMapStatement(Map("@a").WithType(
                                   SizedType(Type::string).WithSize(12)),
                               _,
                               _) })) });
  ...
  test(R"(begin { if (true) { @x = 10; } else { @x = "str"; } })",
       ExpectedAST{ Program().WithProbe(Probe(
           ...
           { ExprStatement(If(
               _,
               Block({ AssignMapStatement(IntMapValue("@x", 1), _, _) }),
               Block(
  ...
  test(R"(begin { let $v; @a = $v + 10; $v = 5; })",
       ExpectedAST{ Program().WithProbe(Probe(
           _,
           { VarDeclStatement(IntVar("$v", 1)),
             AssignMapStatement(IntMapValue("@a", 1),
                                _,
...
TEST_F(TypeGraphTest, map_key_basic)
{
  ...
  test(R"(begin { @a["key"] = 10; })",
       ExpectedAST{ Program().WithProbe(
           Probe(_,
                 { AssignMapStatement(Map("@a").WithKeyType(
                                          SizedType(Type::string).WithSize(4)),
                                      _,
                                      _) })) });
  ...
  test(R"(begin { let $k; @a[$k + 10] = 1; $k = 5; })",
       ExpectedAST{ Program().WithProbe(
           Probe(_,
                 { VarDeclStatement(IntVar("$k", 1)),
                   AssignMapStatement(IntMapKey("@a", 1),
...
TEST_F(TypeGraphTest, variable_map_promotion)
{
  ...
  test(
      R"(begin { $a = 1; @x = (uint32)1; if comptime (typeinfo(@x).2 == "uint64") { $a = (int16)2; } } end { @x = (uint64)2; })",
      ExpectedAST{ Program().WithProbes(
          { Probe(_,
