...
{
  test(input, true, "", warn);
}

TEST(builtins, ctx)
{
    std::vector<std::string> valid = {"uprobe:sh:k", "uretprobe:sh:k", "kprobe:k",
  ...
  test_error("tracepoint:mod:k { $x = ctx; }", "ERROR: Use args instead of ctx in tracepoint");
...
TEST(builtins, args)
{
  std::vector<std::string> valid = {"fentry:k", "fexit:k", "rawtracepoint:k", "uprobe:sh:k"};
...
{
  test("fentry:k { __builtin_retval }");
  test("fexit:k { __builtin_retval }");
  test("kretprobe:k { __builtin_retval }");
  test("uretprobe:sh:k { __builtin_retval }");
...
TEST(builtins, argX)
{
  test("kprobe:k { $x = arg0; $y = arg1; }");
  test("uprobe:sh:k { $x = arg0; $y = arg1; }");
  test("usdt:sh:k { $x = arg0; $y = arg1; }");
...
TEST(builtins, func)
{
  test("kprobe:k { __builtin_func }");
  test("uprobe:sh:k { __builtin_func }");
  test("kretprobe:k { __builtin_func }");
  test("uretprobe:sh:k { __builtin_func }");
  test("fentry:k { __builtin_func }");
