...
Map keys that are composed of multiple types are represented as tuples e.g.
...
For a map with multiple keys, the map and key are combined in the same argument.
...
kprobe:dummy {
  ...
  @associative[1,2] = 1;
  delete(@scalar);
  delete(@associative[1,2]); // This is equivalent: delete(@associative[(1,2)]);
