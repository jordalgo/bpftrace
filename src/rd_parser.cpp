...
ast::Probe *RDParser::parse_probe()
{
  ...
  auto &last_ap_loc = attach_points.back()->loc->current;
...
ast::BlockExpr *RDParser::parse_block(bool allow_trailing_expr)
{
    ...
    if (block) {
      ...
      if (!inner) {
        ...
        auto *es = ctx_.make_node<ast::ExprStatement>(block->loc,
                                                      std::move(block->expr));
        block->stmts.push_back(ast::Statement(es));
  ...
  while (peek() != '}' && !at_end()) {
    ...
    if (check_keyword("import")) {
      ...
      if (imp)
        stmts.push_back(ast::Statement(imp));
      skip_whitespace();
      if (pos_ == before) {
    ...
    if (check_keyword("break")) {
      ...
      stmts.push_back(ast::Statement(jump));
      skip_whitespace();
      continue;
    }
    ...
    if (peek() == '_') {
      ...
      if (p < input_->size() && (*input_)[p] == '=' &&
        ...
        stmts.push_back(ast::Statement(discard));
        skip_whitespace();
        continue;
      }
    }
        ...
        if (es) {
          ...
          if (peek() == '}') {
            ...
            auto *expr_stmt = ctx_.make_node<ast::ExprStatement>(
                loc, std::move(expr));
            stmts.push_back(ast::Statement(expr_stmt));
          }
        } else {
          stmts.push_back(std::move(stmt));
        }
                                                 ...
                                                 int stmt_col)
{
  ...
  return ast::Statement(expr_stmt);
}
...
ast::Expression RDParser::parse_ternary()
{
  ...
  if (peek() == '?') {
    auto &expr_begin = expr.loc()->current.begin;
...
std::optional<BinopInfo> get_binop(const std::string &input, size_t pos)
{
  ...
  if (c == '|' && next == '|')
    return BinopInfo{ ast::Operator::LOR, 1 };
  if (c == '&' && next == '&')
    return BinopInfo{ ast::Operator::LAND, 2 };
  if (c == '=' && next == '=')
    return BinopInfo{ ast::Operator::EQ, 6 };
  if (c == '!' && next == '=')
    return BinopInfo{ ast::Operator::NE, 6 };
  if (c == '<' && next == '=')
    return BinopInfo{ ast::Operator::LE, 7 };
  if (c == '>' && next == '=')
    return BinopInfo{ ast::Operator::GE, 7 };
  if (c == '<' && next == '<')
    return BinopInfo{ ast::Operator::LEFT, 8 };
  if (c == '>' && next == '>')
    return BinopInfo{ ast::Operator::RIGHT, 8 };

  // Single-character operators.
  if (c == '|' && next != '|')
    return BinopInfo{ ast::Operator::BOR, 3 };
  if (c == '^')
    return BinopInfo{ ast::Operator::BXOR, 4 };
  if (c == '&' && next != '&')
    return BinopInfo{ ast::Operator::BAND, 5 };
  if (c == '<' && next != '<' && next != '=')
    return BinopInfo{ ast::Operator::LT, 7 };
  if (c == '>' && next != '>' && next != '=')
    return BinopInfo{ ast::Operator::GT, 7 };
  if (c == '+' && next != '+')
    return BinopInfo{ ast::Operator::PLUS, 9 };
  if (c == '-' && next != '-')
    return BinopInfo{ ast::Operator::MINUS, 9 };
  if (c == '*')
    return BinopInfo{ ast::Operator::MUL, 10 };
  if (c == '/') {
    ...
    if (p < input.size() && input[p] == '{')
      return std::nullopt;
    return BinopInfo{ ast::Operator::DIV, 10 };
  }
  if (c == '%')
    return BinopInfo{ ast::Operator::MOD, 10 };
...
ast::Expression RDParser::parse_binary(int min_prec, ast::Expression left)
{
  while (true) {
    ...
    auto &left_begin = left.loc()->current.begin;
...
ast::Expression RDParser::parse_primary()
{
  ...
  if (peek() == '"') {
    ...
    return ast::Expression(string_node);
  }
...
ast::Expression RDParser::parse_paren_expr()
{
  ...
  // Look ahead: ident followed by '=' (not '==')
  {
    ...
    if (std::isalpha((*input_)[p]) || (*input_)[p] == '_') {
      ...
      if (p < input_->size() && (*input_)[p] == '=' &&
        ...
        std::string ident(&(*input_)[id_start],
                          static_cast<size_t>(p - id_start));
  ...
  if (std::isalpha(peek()) || peek() == '_') {
    ...
    std::string peeked_ident(&(*input_)[pos_], static_cast<size_t>(p - pos_));
    ...
    if (!looks_like_cast) {
      ...
      std::string first_word(&(*input_)[pos_], static_cast<size_t>(p - pos_));
    ...
    if (!looks_like_cast && p < input_->size() && (*input_)[p] == '*') {
      ...
      std::string ident_str(&(*input_)[pos_], static_cast<size_t>(p - pos_));
                                                       ...
                                                       int start_col)
{
  ...
  auto *map_access = ctx_.make_node<ast::MapAccess>(access_loc,
                                                    map,
                                                    std::move(key_expr));
  return ast::Expression(map_access);
}
...
bool RDParser::check_keyword(const std::string &s) const
{
  ...
  if (p + s.size() < input_->size() &&
      (std::isalnum((*input_)[p + s.size()]) || (*input_)[p + s.size()] == '_'))
    return false;
  return true;
}
...
bool RDParser::is_builtin(const std::string &name)
{
  ...
  if (name.size() >= 4 && name.substr(0, 3) == "arg" &&
      std::all_of(name.begin() + 3, name.end(), ::isdigit)) {
    return true;
  }
  if (name.size() >= 5 && name.substr(0, 4) == "sarg" &&
...
bool RDParser::looks_like_type() const
{
  ...
  while (save < input_->size() &&
    ...
    save++;
  std::string ident(&(*input_)[id_start], static_cast<size_t>(save - id_start));
