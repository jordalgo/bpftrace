                                      ...
                                      const Location &loc)
{
  ...
  StructType *stack_struct_type = b_.GetStackStructType(is_ustack,
                                                        stype.stack_type.limit);
  ...
  auto stack_offset = is_ustack ? 3 : 1;
  Value *stack_size = b_.CreateGetStack(
      ctx_,
      is_ustack,
      b_.CreateGEP(stack_struct_type,
                   stack,
                   { b_.getInt64(0), b_.getInt32(stack_offset) }),
      stype.stack_type,
      loc);
  ...
  auto num_frames_offset = is_ustack ? 2 : 0;
  b_.CreateStore(num_frames,
                 b_.CreateGEP(stack_struct_type,
                              stack,
                              { b_.getInt64(0),
                                b_.getInt32(num_frames_offset) }));
...
ScopedExpr CodegenLLVM::visit(Builtin &builtin)
{
  if (builtin.ident == "nsecs") {
    ...
    return kstack_ustack(builtin.ident, builtin.builtin_type, builtin.loc);
...
ScopedExpr CodegenLLVM::visit(Call &call)
{
  if (call.func == "count") {
    ...
    bool is_ustack = arg.type().IsUstackTy();
    auto *stack_struct_type = b_.GetStackStructType(
        is_ustack, arg.type().stack_type.limit);
    auto num_frames_offset = is_ustack ? 2 : 0;
    ...
    return kstack_ustack(call.func, call.return_type, call.loc);
